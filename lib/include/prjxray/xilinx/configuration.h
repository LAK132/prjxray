...
Configuration<ArchType>::createType2ConfigurationPacketData(
	...
	if (!compressed) {
		...
		for (const auto& frame : frames) {
			...
			//     part->GetNextFrameAddress(frame.first);
			// if (next_address &&
			//     !similar_address(frame.first, *next_address)) {
			// 	result.frames.back().data.resize(
			// 	    result.frames.back().data.size() +
...
Configuration<ArchType>::InitWithPackets(const typename ArchType::Part& part,
	...
	for (auto packet : packets) {
		...
		switch (packet.address()) {
			case ArchType::ConfRegType::MASK:
				std::cout << " MASK";
				for (const auto& data : packet.data())
					std::cout << " 0x" << std::hex
					          << std::uppercase
					          << std::setfill('0')
					          << std::setw(8) << data;
			...
			case ArchType::ConfRegType::FDRI: {
				...
				if (start_new_write) {
					...
					std::cout
					    ...
					    << std::uppercase
					    << std::setfill('0') << std::setw(8)
					    << uint32_t(current_frame_address)
					    << " SIZE: 0x" << std::hex
					    << std::uppercase
					    << std::setfill('0') << std::setw(8)
					    << packet.data().size();
